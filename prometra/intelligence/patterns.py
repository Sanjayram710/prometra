...
class PatternDetector:
    ...
    def detect_patterns(
        ...
        doc_count = sum(
            1
            for item in top_files
            if item.get("path", "").endswith((".md", ".txt", ".rst"))
        )
        if doc_count > 0 or any(
            "doc" in msg.lower() for msg in commit_messages + prompts
        ):
            patterns.append(
                CodingPattern(
        ...
        if any(
            w in msg.lower()
            for msg in commit_messages + prompts
            for w in ("fix", "bug", "issue", "patch", "error")
        ):
            patterns.append(
                CodingPattern(
        ...
        if files_created > 0 or any(
            kw in msg.lower()
            for msg in (commit_messages + prompts)
            for kw in ("feat", "add", "feature", "create")
        ):
            patterns.append(
                CodingPattern(
