...
from prometra.context.models import (
    Context, ProjectState, RepositorySummary, SessionSummary, 
    GitSnapshot, FileChange, AnalyzerSummary, TimelineSummary
)
...
class ContextBuilder:
    ...
    def build_context(self, project_id: str, project_path: str, session_id: Optional[str] = None) -> Context:
        db = self.storage.get_session()
        try:
            # 1. Repository Summary
            repo_summary = RepositorySummary(project_id=project_id, root_path=project_path)
            
            # 2. Session Summary
            session_summary = None
            if session_id:
                s_model = db.query(SessionModel).filter_by(session_id=session_id).first()
                if s_model:
                ...
                s_model = db.query(SessionModel).filter_by(project_id=project_id, status="active").first()
                if s_model:
                    session_summary = SessionSummary(
                        session_id=s_model.session_id,
                        duration_seconds=s_model.duration_seconds or 0,
            ...
            # 3. Git Snapshot
            git_snapshot = None
            git_events = db.query(GitEventModel).order_by(GitEventModel.timestamp.desc()).limit(1).all()
            if git_events:
            ...
            fs_events = fs_query.order_by(FilesystemEventModel.timestamp.desc()).limit(10).all()
            for f in fs_events:
                recent_files.append(FileChange(
            ...
            timeline_summary = TimelineSummary(
                ...
                recent_events=[{"time": str(e.timestamp), "type": e.normalized_event_type, "summary": e.summary} for e in timeline_events]
            )
