...
class StatsCalculator:
    ...
    def compute_project_stats(self, project_id: str):
        ...
        try:
            ...
            fs_events = db.query(FilesystemEventModel).filter_by(project_id=project_id).count()
            
            # Since git events don't have project_id directly, we find via timeline
            git_events_count = 0
            # For V1, we just count timeline events with type git
            git_events_count = db.query(TimelineEventModel).filter_by(normalized_event_type="git").count()

            langs = {}
            for fs in db.query(FilesystemEventModel).filter_by(project_id=project_id, operation="modified").all():
            ...
            return {
                ...
                "dependency_changes": langs.get("json", 0) + langs.get("toml", 0) + langs.get("yaml", 0)
            }
        finally:
            db.close()
