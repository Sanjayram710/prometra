...
class CompareEngine:
    ...
    def resolve_latest_sessions(self) -> tuple[str, str]:
        ...
        try:
            ...
            sessions = (
                db.query(SessionModel).order_by(SessionModel.start_ts.desc()).all()
            )
            ...
            if len(sess_ids) < 2:
                raise ValueError(
                    "At least two sessions are required to compare using --latest."
                )
    ...
    def get_session_stats(self, session_id: str) -> SessionStats:
        ...
        try:
            ...
            # Filesystem metrics
            fs_events = (
                db.query(FilesystemEventModel).filter_by(session_id=session_id).all()
            )
            files_created = sum(
                1
                for e in fs_events
                ...
                for e in tl_events:
                    ...
                    if "filesystem" in net or "file" in net:
                        ...
                        if "created" in sum_lower or "create" in sum_lower:
                            ...
                            "deleted" in sum_lower
                            or "delete" in sum_lower
                            or "removed" in sum_lower
                        ):
                            files_deleted += 1
                        else:
                            files_modified += 1
            ...
            git_events_session = sum(
                1 for e in tl_events if "git" in (e.normalized_event_type or "").lower()
            )
            if git_events_session > 0:
                ...
                git_commits = sum(
                    1
                    for e in tl_events
                    if "commit" in (e.summary or "").lower()
                    or "git" in (e.source or "").lower()
                )

            # AI metrics
            ai_events_count = (
                db.query(AiEventModel).filter_by(session_id=session_id).count()
            )
            if ai_events_count == 0:
                ai_events_count = sum(
                    ...
                    for e in tl_events
                    if any(
                        k in (e.normalized_event_type or "").lower()
                        for k in ["ai", "prompt", "response", "tool", "model", "token"]
                    )
            ...
            productivity = {
                ...
                "files_changed_per_minute": round(
                    (files_created + files_modified + files_deleted)
                    / duration_minutes_non_zero,
                    2,
                ),
                "commits_per_hour": round(
                    (git_commits * 60) / duration_minutes_non_zero, 2
                ),
            }

            start_ts_str = (
                str(sess_model.start_ts)
                if sess_model and sess_model.start_ts
                else (str(tl_events[0].timestamp) if tl_events else None)
            )
    ...
    def compare_sessions(
        ...
        if not session_a or not session_b:
            raise ValueError(
                "Two session IDs are required for comparison (or use --latest)."
            )
        ...
        dur_diff_str = (
            f"+{dur_min_diff} min" if dur_min_diff >= 0 else f"{dur_min_diff} min"
        )
