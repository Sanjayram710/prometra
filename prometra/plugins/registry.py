...
from prometra.plugins.exceptions import DuplicatePluginError, PluginNotFoundError
...
class PluginRegistry:
    ...
    def register(self, plugin_cls: Type[BasePlugin], overwrite: bool = False) -> None:
        ...
        if not issubclass(plugin_cls, BasePlugin):
            raise ValueError(f"Class '{plugin_cls}' must be a subclass of BasePlugin.")
        ...
        if name in self._plugins and not overwrite:
            raise DuplicatePluginError(f"Plugin with name '{name}' is already registered.")
