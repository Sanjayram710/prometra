...
class EventTranslatorRegistry:
    ...
    def translate(
        self, connector_name: str, event: BaseEvent
    ) -> ai_ev.AiEvent | None:
...
def claude_event_translator(event: BaseEvent) -> ai_ev.AiEvent | None:
    if isinstance(event, cl_ev.ClaudeSessionStarted):
        ...
        return ai_ev.PromptUpdated(
            connector_name="claude", session_id=event.session_id, content=event.content
        )
        ...
        tokens = TokenCount(
            ...
            or (event.prompt_tokens + event.completion_tokens),
        )
        return ai_ev.TokenUsage(
            connector_name="claude", session_id=event.session_id, tokens=tokens
        )
    elif isinstance(event, cl_ev.ClaudeCostRecorded):
        return ai_ev.CostRecorded(
            connector_name="claude", session_id=event.session_id, cost=event.cost
        )
        ...
        return ai_ev.ConnectorConnected(connector_name="claude", session_id="system")
    elif isinstance(event, cl_ev.ClaudeDisconnected):
        return ai_ev.ConnectorDisconnected(
            connector_name="claude", session_id="system", reason=event.reason
        )
    elif isinstance(event, cl_ev.ClaudeHealthChanged) and not event.is_healthy:
        return ai_ev.ErrorOccurred(
                connector_name="claude",
                session_id="system",
