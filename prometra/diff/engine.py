...
from prometra.diff.models import DiffResult, FileVersion
...
class DiffEngine:
    ...
    def _extract_content_from_event(
        ...
        fs_event: FilesystemEventModel | None = None,
        ai_event: AiEventModel | None = None,
        git_event: GitEventModel | None = None,
    ...
    def get_file_events(
        ...
        try:
            ...
            for r in fs_records:
                ...
                if (
                    r_norm == target_norm
                    or r_path == target_norm
                    or r_norm.endswith(target_norm)
                    or r_filename == filename
                ):
            ...
            for r in ai_records:
                ...
                if r.extra_metadata and isinstance(r.extra_metadata, dict):
                    p = (
                        r.extra_metadata.get("path")
                        or r.extra_metadata.get("file")
                        or r.extra_metadata.get("file_path")
                    )
                    if p:
                        p_norm = self._normalize_path(str(p))
                        if (
                            p_norm == target_norm
                            or p_norm.endswith(target_norm)
                            or os.path.basename(str(p)).lower() == filename
                        ):
            ...
            if not file_versions:
                for fs_id in sorted(matching_fs_ids):
                    fs_rec = fs_map.get(fs_id)
                    if fs_rec:
                        content = self._extract_content_from_event(
                            tl_event=None, fs_event=fs_rec, file_path=file_path
                        )
                        v = FileVersion(
    ...
    def resolve_version_pair(
        ...
        if not versions:
            if session_id:
                raise ValueError(
                    f"Session '{session_id}' not found or has no events for '{file_path}'."
                )
