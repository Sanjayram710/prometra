...
class DashboardRenderer:
    ...
    def render(self, metrics: DashboardMetrics):
        ...
        long_str = (
            f"{sess.longest_session_seconds // 60}m"
            if sess.longest_session_seconds >= 60
            else f"{sess.longest_session_seconds}s"
        )
        avg_str = (
            f"{sess.avg_session_length_seconds // 60}m"
            if sess.avg_session_length_seconds >= 60
            else f"{sess.avg_session_length_seconds}s"
        )
        ...
        self.console.print(
            Panel(
                overview_table,
                title="[bold white]📊 Sessions & Overview[/bold white]",
                border_style="blue",
            )
        )
        ...
        fs_table.add_row(
            "Files Created", f"[green]{metrics.filesystem.files_created}[/green]"
        )
        fs_table.add_row(
            "Files Modified", f"[yellow]{metrics.filesystem.files_modified}[/yellow]"
        )
        fs_table.add_row(
            "Files Deleted", f"[red]{metrics.filesystem.files_deleted}[/red]"
        )
        fs_table.add_row(
            "Git Commits",
            f"[blue]{metrics.git.total_commits} ({metrics.git.commits_per_day} / day)[/blue]",
        )

        self.console.print(
            Panel(
                fs_table,
                title="[bold white]📝 Filesystem & Git Activity[/bold white]",
                border_style="green",
            )
        )
        ...
        ai_table.add_row(
            "AI Prompts", f"[bright_cyan]{metrics.ai.ai_prompts}[/bright_cyan]"
        )
        ...
        ai_table.add_row(
            "Errors / Retries",
            f"[red]{metrics.ai.errors}[/red] / [yellow]{metrics.ai.retries}[/yellow]",
        )
        ai_table.add_row(
            "Total Tokens",
            f"[magenta]{metrics.ai.total_tokens} (In: {metrics.ai.prompt_tokens}, Out: {metrics.ai.completion_tokens})[/magenta]",
        )
        ai_table.add_row(
            "Estimated Cost",
            f"[bold green]${metrics.ai.estimated_cost:.4f}[/bold green]",
        )
        ai_table.add_row(
            "Connectors Used",
            f"[yellow]{', '.join(metrics.ai.connectors_used) if metrics.ai.connectors_used else 'None'}[/yellow]",
        )

        self.console.print(
            Panel(
                ai_table,
                title="[bold white]🤖 AI Interactions & Costs[/bold white]",
        ...
        if metrics.filesystem.top_edited_files:
            file_table = Table(
                title="Top Edited Files", show_header=True, header_style="bold green"
            )
        ...
        if metrics.ai.top_models:
            model_table = Table(
                title="Top AI Models", show_header=True, header_style="bold magenta"
            )
        ...
        if metrics.activity.top_active_hours:
            ...
            self.console.print(
                Panel(
                    f"[bold white]Peak Active Hours:[/bold white] [cyan]{hours_str}[/cyan]",
                    border_style="cyan",
                )
            )
