...
from prometra.storage.models import SessionModel, FilesystemEventModel, GitEventModel, TimelineEventModel, AiEventModel
from prometra.dashboard.metrics import (
    DashboardMetrics, SessionMetrics, FilesystemMetrics,
    GitMetrics, AiMetrics, ActivityMetrics, TopFileEntry, TopModelEntry
)
...
class DashboardEngine:
    ...
    def compute_metrics(
        ...
        try:
            ...
            now = utcnow()
            start_boundary: Optional[datetime.datetime] = None
            ...
            durations = [s.duration_seconds for s in sessions_list if s.duration_seconds]
            ...
            if start_boundary:
                fs_query = fs_query.filter(FilesystemEventModel.timestamp >= start_boundary)
            if session_id:
            ...
            ai_prompts = sum(1 for r in ai_records if "prompt" in (r.event_type or "").lower())
            ai_responses = sum(1 for r in ai_records if "response" in (r.event_type or "").lower())
            tool_calls = sum(1 for r in ai_records if "tool" in (r.event_type or "").lower())
            errors = sum(1 for r in ai_records if "error" in (r.event_type or "").lower() or "fail" in (r.event_type or "").lower())
            retries = sum(1 for r in ai_records if "retry" in (r.event_type or "").lower())
            ...
            top_models = [TopModelEntry(model_name=k, count=v) for k, v in sorted_models]

            # 5. Activity Pattern Metrics
            tl_query = db.query(TimelineEventModel)
            if start_boundary:
                tl_query = tl_query.filter(TimelineEventModel.timestamp >= start_boundary)
            if session_id:
            ...
            top_active_hours = sorted(hour_counts.keys(), key=lambda k: hour_counts[k], reverse=True)[:3]
