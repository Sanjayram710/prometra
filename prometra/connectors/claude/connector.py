...
class ClaudeConnector(BaseConnector):
    ...
    def connect(self) -> None:
        ...
        self.emit_event(
            ClaudeConnected(
                version=ClaudeDiscovery.get_version() or "1.0.0",
    ...
    def _persist_event(self, event: Any, related_id: str | None = None):
    ...
    def metadata(self) -> ClaudeMetadata:
        ...
        _version = ClaudeDiscovery.get_version()
    ...
    def health(self) -> ConnectorStatus:
        if not ClaudeDiscovery.is_installed():
            return ConnectorStatus(
                state="error", error_message="Claude Code is not installed."
            )
