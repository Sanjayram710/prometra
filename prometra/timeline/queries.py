...
class TimelineQueryEngine:
    ...
    def build_query(self, db_session, filters: TimelineFilter):
        ...
        if filters.reverse:
            query = query.order_by(
                TimelineEventModel.sequence.desc(), TimelineEventModel.id.desc()
            )
        else:
            query = query.order_by(
                TimelineEventModel.sequence.asc(), TimelineEventModel.id.asc()
            )
    ...
    def fetch_summary(self, filters: TimelineFilter) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                not sessions
                and not filters.session_id
                and not filters.search
                and not filters.event_type
            ):
    ...
    def fetch_grouped_by_session(self, filters: TimelineFilter) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for sess_id, sess_events in grouped.items():
                sess_model = (
                    db.query(SessionModel).filter_by(session_id=sess_id).first()
                    if sess_id != "unassigned"
                    else None
                )

                duration = (
                    sess_model.duration_seconds
                ...
                files_changed = sum(
                    ...
                    if "filesystem" in (e.normalized_event_type or "").lower()
                )
                git_commits = sum(
                    1
                    for e in sess_events
                    if "git" in (e.normalized_event_type or "").lower()
                )
                ai_events = sum(
                    1
                    for e in sess_events
