...
from prometra.storage.models import TimelineEventModel, AiEventModel, SessionModel
...
def test_claude_token_and_cost_and_error(ai_environment):
    ...
    cost_ev = cl_ev.ClaudeCostRecorded(
        session_id="sess-ai-2",
        cost=0.0125
    )
...
def test_connector_connected_and_disconnected(ai_environment):
    ...
    conn_ev = cl_ev.ClaudeConnected(version="1.0.0", executable_path="/usr/local/bin/claude")
...
def test_timeline_cli_ai_filtering_and_search(monkeypatch, ai_environment):
    ...
    # Emit events
    connector.emit_event(cl_ev.ClaudePromptSubmitted(session_id="sess-cli", content="Refactor JWT authentication module"))
    connector.emit_event(cl_ev.ClaudeResponseReceived(session_id="sess-cli", content="Refactored JWT module successfully", prompt_tokens=100, completion_tokens=100))
    connector.emit_event(cl_ev.ClaudeToolInvocationStarted(session_id="sess-cli", tool_name="Write File", arguments={"path": "auth.py"}))
    ...
    assert "PromptSubmitted" in res_search_prompt.stdout or "Prompt Submitted" in res_search_prompt.stdout

    # Test --search tool
    res_search_tool = runner.invoke(app, ["timeline", "--search", "tool"])
    assert res_search_tool.exit_code == 0
    assert "ToolInvocation" in res_search_tool.stdout or "Tool Invocation" in res_search_tool.stdout
