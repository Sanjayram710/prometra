...
from prometra.timemachine.models import FileSnapshot, CheckpointModel, RestorePreview, CheckpointDiff
...
def test_checkpoint_timeline(temp_tm_env):
    tmpdir, storage = temp_tm_env
    mgr = CheckpointManager(storage=storage, root_dir=tmpdir)
    cp = mgr.create_checkpoint(message="Timeline CP")
...
def test_cli_timemachine_commands(monkeypatch, temp_tm_env):
    ...
    cps = CheckpointStorage(root_dir=os.path.join(tmpdir, ".prometra")).list_checkpoints()
    assert len(cps) >= 2
