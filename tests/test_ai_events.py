import pytest
from prometra.connectors.claude.events import ClaudeSessionStarted, ClaudeHealthChanged
from prometra.ai.translators import ai_translator_registry
from prometra.ai.events import SessionStarted, ErrorOccurred
from prometra.ai.metrics import AiMetricsAggregator
from prometra.ai.events import TokenUsage, LatencyMeasured, ToolInvocation
...
def test_metrics_aggregator():
    events = [
        TokenUsage(connector_name="test", session_id="1", tokens=TokenCount(prompt_tokens=10, completion_tokens=20, total_tokens=30)),
        LatencyMeasured(connector_name="test", session_id="1", latency_ms=100, operation="completion"),
        LatencyMeasured(connector_name="test", session_id="1", latency_ms=200, operation="completion"),
        ToolInvocation(connector_name="test", session_id="1", tool=ToolCall(tool_name="test_tool", arguments={}))
    ]
