...
def test_cli_replay_commands(monkeypatch, populated_session_engine):
    monkeypatch.setattr("prometra.cli.commands.get_storage", lambda: populated_session_engine.storage)
    
    # Test prometra replay --latest
    res_latest = runner.invoke(app, ["replay", "--latest"])
    assert res_latest.exit_code == 0
    assert "sess-replay-100" in res_latest.stdout or "Session Replay" in res_latest.stdout
