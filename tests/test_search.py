...
def test_session_filter(populated_search_engine):
    ...
    res_invalid = populated_search_engine.search_events(
        "hello", session="sess-non-existent"
    )
...
def test_date_range_filters(populated_search_engine):
    ...
    res_range = populated_search_engine.search_events(
        "auth", since=since_str, until=until_str
    )
...
def test_invalid_date_and_session_handling(populated_search_engine):
    ...
    res_bad_date = populated_search_engine.search_events(
        "auth", since="not-a-valid-date", until="invalid-until"
    )
    assert res_bad_date.total_results >= 1

    # Non-existent session ID should return 0 results
    res_bad_sess = populated_search_engine.search_events(
        "auth", session="sess-invalid-999999"
    )
...
def test_cli_search_command(monkeypatch, populated_search_engine):
    monkeypatch.setattr(
        "prometra.cli.commands.get_storage", lambda: populated_search_engine.storage
    )
